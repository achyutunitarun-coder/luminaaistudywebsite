...
export function searchFiles(pattern: string, cwd: string): string[] {
  try { return require('glob').sync(pattern, { cwd, nodir: true }); } catch { return []; }
}
...
export function grep(pattern: string, path: string): string[] {
  try {
    ...
    return lines.map((line, i) => ({ line, num: i + 1 })).filter(({ line }) => regex.test(line)).map(({ line, num }) => num + ': ' + line);
  } catch { return []; }
}
...
export function npmCmd(args: string, cwd: string): string {
