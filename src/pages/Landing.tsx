...
/* ═══════════════════════════════════════════════════════════════
   TOKENS — matches the design system
   ═══════════════════════════════════════════════════════════════ */
const C = {
  bg: '#07080d',
  surface: '#0d1018',
      ...
      <div className="max-w-[1180px] mx-auto px-6 h-[64px] flex items-center justify-between">
        ...
        <div className="flex items-center gap-2.5">
          <div
            ...
            style={{ width: 28, height: 28, background: `linear-gradient(135deg, ${C.teal}, ${C.violet})` }}
          >
            <Sparkles className="text-black" style={{ width: 14, height: 14 }} />
          </div>
          <span
            className="font-medium tracking-tight"
            style={{ fontFamily: "'Instrument Serif', serif", fontSize: 22, color: C.ink, lineHeight: 1 }}
          >
            Lumina
          </span>
        </div>
        ...
        <div className="hidden md:flex items-center gap-3">
          <button
            ...
            style={{ color: C.ink, background: 'none', border: 'none', cursor: 'pointer' }}
          >
            Sign in
          </button>
          <button
            onClick={() => navigate('/auth')}
            className="inline-flex items-center justify-center gap-1.5 rounded-[10px] px-5 h-10 text-[13px] font-semibold transition-all hover:opacity-90 hover:translate-y-[-1px]"
            style={{
              background: C.ink,
              color: C.bg,
              border: 'none',
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            style={{ borderColor: C.line, background: 'rgba(7,8,13,0.96)' }}
          >
            {['Features', 'How it works', 'Pricing', 'FAQ'].map(item => (
              <a key={item} href={`#${item.toLowerCase().replace(/\s+/g, '-')}`} onClick={() => setOpen(false)} className="text-[14px]" style={{ color: C.inkMute }}>
                {item}
              </a>
            ))}
    ...
    <section ref={ref} className="relative pt-[140px] pb-[80px] overflow-hidden">
      ...
      <div className="absolute inset-0 -z-10 pointer-events-none">
        ...
        <div
          ...
          style={{
            backgroundImage: 'linear-gradient(rgba(255,255,255,0.5) 0.5px, transparent 0.5px), linear-gradient(90deg, rgba(255,255,255,0.5) 0.5px, transparent 0.5px)',
      ...
      <motion.div style={{ y, opacity }} className="max-w-[1180px] mx-auto px-6 text-center">
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease }} className="flex justify-center mb-6">
          <div
            ...
            style={{ color: C.inkMute, background: 'rgba(255,255,255,0.03)', border: `0.5px solid ${C.lineStrong}`, letterSpacing: '0.02em' }}
          >
        ...
        <motion.h1
          ...
          style={{ fontFamily: "'Instrument Serif', serif", fontSize: 'clamp(48px, 7vw, 92px)', color: C.ink, fontWeight: 400 }}
        >
          The quiet intelligence
          <br />
          <span
            style={{
              background: `linear-gradient(110deg, ${C.teal} 0%, ${C.violet} 50%, ${C.amber} 100%)`,
        ...
        {/* CTA row */}
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, ease, delay: 0.25 }} className="mt-9 flex items-center justify-center gap-2.5 flex-wrap">
          <button
            onClick={() => navigate('/auth')}
            className="group relative inline-flex items-center justify-center gap-1.5 rounded-[10px] px-5 h-10 text-[13px] font-semibold transition-all hover:opacity-90 hover:translate-y-[-1px]"
            style={{ background: C.ink, color: C.bg, border: 'none', cursor: 'pointer', boxShadow: '0 1px 0 rgba(255,255,255,0.15) inset, 0 8px 24px -8px rgba(45,212,191,0.35)' }}
          >
            Start learning free <ArrowRight className="w-3.5 h-3.5" />
          </button>
          <button
            onClick={() => document.getElementById('how')?.scrollIntoView({ behavior: 'smooth' })}
            className="inline-flex items-center justify-center gap-1.5 rounded-[10px] px-5 h-10 text-[13px] font-medium transition-all hover:bg-white/[0.06]"
            style={{ color: C.ink, border: `0.5px solid ${C.lineStrong}`, background: 'none', cursor: 'pointer' }}
          >
        ...
        {/* Social proof */}
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5, duration: 0.8 }} className="mt-5 text-[12px]" style={{ color: C.inkFaint }}>
          Free forever · No credit card · Trusted by students at IITs, MIT, Oxford
        </motion.div>

        {/* Product preview */}
        <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, ease, delay: 0.4 }} className="relative mt-16 mx-auto max-w-[1080px]">
          <div
          ...
          <div className="absolute inset-x-10 -bottom-10 h-40 -z-10 blur-[80px] opacity-50" style={{ background: `linear-gradient(90deg, ${C.teal}, ${C.violet})` }} />
        </motion.div>
      </motion.div>
    </section>
  );
...
/* ═══════════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════════ */
const LogoStrip = () => (
  <section className="py-12 border-y" style={{ borderColor: C.line }}>
    <div className="max-w-[1180px] mx-auto px-6">
      <div className="text-center text-[11px] uppercase tracking-[0.2em] mb-6" style={{ color: C.inkFaint }}>Trusted by students preparing for</div>
      <div className="flex flex-wrap justify-center items-center gap-x-12 gap-y-4 text-[20px]" style={{ color: C.inkMute, fontFamily: "'Instrument Serif', serif", fontStyle: 'italic' }}>
        {['JEE', 'NEET', 'SAT', 'IB', 'A-Levels', 'AP', 'GCSE', 'CBSE'].map(t => (
          <span key={t} className="opacity-70 hover:opacity-100 transition-opacity">{t}</span>
        ))}
      </div>
    </div>
  </section>
...
const SectionHeader = ({ eyebrow, title, subtitle }: { eyebrow: string; title: string; subtitle?: string }) => (
  <div className="max-w-[680px]">
    {eyebrow && <div className="text-[11px] uppercase tracking-[0.18em] mb-3" style={{ color: C.teal }}>{eyebrow}</div>}
    <h2 className="text-3xl md:text-4xl font-semibold tracking-[-0.02em] leading-[1.15]" style={{ color: C.ink }}>{title}</h2>
    {subtitle && <p className="mt-4 text-[15px] md:text-[16px] leading-[1.6]" style={{ color: C.inkMute }}>{subtitle}</p>}
  </div>
);
...
const FeatureCard = ({ icon: Icon, color, title, desc, visual, span = 'md:col-span-4', tall }: {
  ...
  <div
    className={`group relative rounded-[20px] p-6 md:p-8 overflow-hidden ${span} ${tall ? 'min-h-[340px]' : ''}`}
    style={{ background: 'linear-gradient(180deg, rgba(255,255,255,0.03), rgba(255,255,255,0.01))', border: `0.5px solid ${C.line}` }}
  >
    <div className="absolute top-0 right-0 w-[200px] h-[200px] rounded-full opacity-[0.06] blur-[60px]" style={{ background: color }} />
    <div className="relative z-10">
      <div className="w-10 h-10 rounded-[10px] flex items-center justify-center mb-4" style={{ background: `${color}15`, border: `1px solid ${color}30` }}>
        <Icon className="w-5 h-5" style={{ color }} />
      </div>
      <h3 className="text-[17px] font-semibold mb-2" style={{ color: C.ink }}>{title}</h3>
      <p className="text-[14px] leading-[1.6] max-w-[420px]" style={{ color: C.inkMute }}>{desc}</p>
    ...
    {[
      ...
      <motion.div key={item.topic} initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.8 + i * 0.1, duration: 0.5 }}>
        <div className="flex items-center justify-between mb-1">
          <span className="text-[12px]" style={{ color: C.ink2 }}>{item.topic}</span>
          <span className="text-[12px] font-medium tabular-nums" style={{ color: item.color }}>{item.pct}%</span>
        </div>
...
const SourcesVisual = () => (
  <div
    className="rounded-[14px] p-6 max-w-[320px] text-center"
    style={{ border: `1px dashed ${C.lineStrong}`, background: 'rgba(255,255,255,0.02)' }}
  >
    <div className="flex justify-center gap-3 mb-3">
      {[
        ...
        <div key={s.label} className="w-12 h-12 rounded-[10px] flex items-center justify-center" style={{ background: 'rgba(255,255,255,0.04)', border: `0.5px solid ${C.line}` }}>
          <s.icon className="w-5 h-5" style={{ color: C.inkMute }} />
        </div>
      ))}
    </div>
    <div className="text-[12px]" style={{ color: C.inkFaint }}>Drop any source — Lumina reads it all</div>
  </div>
);
...
const HowItWorks = () => {
  ...
  return (
    <section id="how" className="py-32" style={{ background: C.surface }}>
      <div className="max-w-[1180px] mx-auto px-6">
        <SectionHeader
          eyebrow="How it works"
          title="Three steps. Then it runs itself."
          subtitle="No setup. Drop in your material, ask a question, and Lumina builds your study map."
        />
        <div className="mt-16 grid grid-cols-1 md:grid-cols-3 gap-6">
          {steps.map((step, i) => (
            <motion.div
              ...
              style={{ background: 'rgba(255,255,255,0.02)', border: `0.5px solid ${C.line}` }}
            >
              <div className="flex items-center gap-3 mb-4">
                <span className="text-[11px] font-semibold tabular-nums" style={{ color: C.teal, fontFamily: "'JetBrains Mono', monospace" }}>{step.n}</span>
                <div className="w-8 h-8 rounded-[8px] flex items-center justify-center" style={{ background: `${C.teal}10`, border: `1px solid ${C.teal}20` }}>
                  <step.icon className="w-4 h-4" style={{ color: C.teal }} />
                </div>
              </div>
              <h3 className="text-[17px] font-semibold mb-2" style={{ color: C.ink }}>{step.title}</h3>
              <p className="text-[14px] leading-[1.6]" style={{ color: C.inkMute }}>{step.desc}</p>
  ...
  <section className="py-32">
    <div className="max-w-[1180px] mx-auto px-6">
      <SectionHeader
        ...
        title="Built for real classrooms, not a launch headline."
      />
        ...
        {[
          ...
          >
            <div className="flex items-center gap-1 mb-4">
              {[...Array(parseInt(t.score.split('/')[0]))].map((_, j) => (
                <Star key={j} className="w-3.5 h-3.5" style={{ color: C.amber, fill: C.amber }} />
              ))}
              <span className="ml-2 text-[11px] font-medium" style={{ color: C.inkFaint }}>{t.score}</span>
            </div>
            <p className="text-[16px] leading-[1.65] mb-6 italic" style={{ color: C.ink2, fontFamily: "'Instrument Serif', serif" }}>"{t.quote}"</p>
            <div className="text-[12px]" style={{ color: C.inkFaint }}>— {t.author}</div>
          </motion.div>
        ))}
      </div>
      <p className="text-center text-[12px] mt-6" style={{ color: C.inkFaint }}>
...
const Pricing = () => {
  ...
  return (
    <section id="pricing" className="py-32" style={{ background: C.surface }}>
      <div className="max-w-[1180px] mx-auto px-6">
        <SectionHeader
          eyebrow="Pricing"
        ...
        <div className="mt-16 grid grid-cols-1 md:grid-cols-3 gap-4">
          {plans.map((plan, i) => (
            <motion.div
              ...
              className="relative rounded-[20px] p-8"
              style={{
                background: plan.featured ? `linear-gradient(180deg, rgba(168,85,247,0.08), rgba(168,85,247,0.02))` : 'rgba(255,255,255,0.02)',
                border: plan.featured ? `1px solid rgba(168,85,247,0.3)` : `0.5px solid ${C.line}`,
              }}
            >
              {plan.featured && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 rounded-full text-[10px] font-semibold" style={{ background: C.violet, color: '#fff' }}>
                  Most popular
                </div>
              )}
              <div className="text-[13px] font-semibold mb-1" style={{ color: C.ink }}>{plan.name}</div>
              <div className="flex items-baseline gap-1 mb-1">
                <span className="text-[36px] font-semibold tracking-[-0.02em]" style={{ color: C.ink }}>{plan.price}</span>
                <span className="text-[13px]" style={{ color: C.inkFaint }}>{plan.period}</span>
              </div>
              <div className="text-[13px] mb-6" style={{ color: C.inkMute }}>{plan.desc}</div>
              <button
                onClick={() => navigate('/auth')}
                className="w-full h-10 rounded-[10px] text-[13px] font-semibold transition-all hover:opacity-90"
                style={{
                  background: plan.featured ? C.ink : 'rgba(255,255,255,0.06)',
                  color: plan.featured ? C.bg : C.ink,
                  border: 'none',
                  cursor: 'pointer',
                }}
              >
              ...
              <div className="mt-6 space-y-2">
                {plan.features.map(f => (
                  <div key={f} className="flex items-center gap-2 text-[13px]" style={{ color: C.inkMute }}>
...
const CTASection = () => {
  ...
  return (
    <section className="py-32">
      <div className="max-w-[1180px] mx-auto px-6">
        <div
          className="relative rounded-[24px] overflow-hidden p-12 md:p-20 text-center"
          style={{ background: `linear-gradient(135deg, #0f1f3d, #1a0d2e)` }}
        >
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="relative z-10 max-w-2xl mx-auto">
            <h2 className="text-3xl md:text-4xl font-semibold mb-4" style={{ color: C.ink }}>Ready to learn smarter?</h2>
            <p className="text-[15px] mb-8 leading-relaxed" style={{ color: C.inkMute }}>
              Join thousands of students who are already using Lumina to master their subjects. Start free, upgrade when you're ready.
            </p>
            <div className="flex gap-3 justify-center flex-wrap">
              <button
                onClick={() => navigate('/auth')}
                className="inline-flex items-center justify-center gap-1.5 rounded-[10px] px-6 h-11 text-[14px] font-semibold transition-all hover:opacity-90 hover:translate-y-[-1px]"
                style={{ background: C.ink, color: C.bg, border: 'none', cursor: 'pointer' }}
              >
                Start learning free <ArrowRight className="w-4 h-4" />
              </button>
              <button
                onClick={() => document.getElementById('pricing')?.scrollIntoView({ behavior: 'smooth' })}
                className="inline-flex items-center justify-center gap-1.5 rounded-[10px] px-6 h-11 text-[14px] font-medium transition-all hover:bg-white/[0.06]"
                style={{ color: C.ink, border: `0.5px solid ${C.lineStrong}`, background: 'none', cursor: 'pointer' }}
              >
                View pricing
              </button>
...
/* ═══════════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════════ */
const Footer = () => (
  <footer className="border-t py-12" style={{ borderColor: C.line }}>
    <div className="max-w-[1180px] mx-auto px-6">
      <div className="flex flex-col md:flex-row items-center justify-between gap-6">
        <div className="flex items-center gap-2.5">
          <div className="w-7 h-7 rounded-[6px] flex items-center justify-center" style={{ background: `linear-gradient(135deg, ${C.teal}, ${C.violet})` }}>
            <Sparkles className="w-3.5 h-3.5 text-black" />
          </div>
