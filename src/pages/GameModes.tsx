...
import { Gamepad2, Swords, Zap, Brain, Shield, ArrowLeft, Trophy, Timer, CheckCircle2, XCircle, Loader2 } from 'lucide-react';
...
const GAME_MODES: GameMode[] = [
  {
    id: 'neural_heist',
    name: 'Neural Heist',
    description: 'Crack the code by answering questions on a grid. Each correct answer reveals a tile.',
  ...
  },
  {
    ...
    rules: 'Take turns answering questions. Score more than the AI to win.',
  },
  {
    id: 'velocity_run',
    name: 'Velocity Run',
    ...
    rules: '60 seconds. Each correct answer = points. Streak bonuses for consecutive correct answers.',
  },
  {
    id: 'mind_dungeon',
    name: 'Mind Dungeon',
    ...
    rules: '5 floors. Each floor has harder questions. 3 wrong answers = game over.',
  },
];
...
const GameModes = () => {
  ...
  const handleAnswer = (answerIdx: number) => {
    ...
    setTimeout(() => {
      if (correct) {
        setScore(s => s + (selectedMode === 'velocity_run' ? 10 + streak * 5 : 10));
        setStreak(s => s + 1);
      ...
      if (currentQ + 1 >= questions.length) {
        ...
        // Floor progression for mind dungeon
    ...
    return (
      <div className="p-4 md:p-8 max-w-4xl mx-auto space-y-6">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease }}>
          ...
          <p className="text-muted-foreground">Learn through play — pick a mode and challenge yourself</p>
        </motion.div>
          ...
          {GAME_MODES.map((gm, i) => (
            ...
            >
              <Card
                ...
                onClick={() => { setSelectedMode(gm.id); setGameState('setup'); }}
              >
  ...
  if (gameState === 'finished') {
    return (
      <div className="p-4 md:p-8 max-w-2xl mx-auto flex flex-col items-center justify-center min-h-[60vh] space-y-6">
        <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="text-center space-y-4">
          <Trophy className="w-16 h-16 text-yellow-400 mx-auto" />
          <h1 className="text-3xl font-display font-bold text-foreground">Game Over!</h1>
