...
import { Flame, Coins, Sparkles, LogOut, Menu, X, Clock, Crown, Home, BookOpen, Gamepad2, FolderOpen, UserCircle, Wrench, Zap, Brain } from 'lucide-react';
...
const aiToolRoutes = ['/ai-tools', '/chat', '/doubt-solver', '/notes-generator', '/note-to-quiz', '/quick-study', '/lecture-ai', '/smart-notebook', '/flashcards', '/tests'];
const hubRoutes = ['/hub'];
...
function isRouteActive(url: string, pathname: string) {
  ...
  if (url === '/ai-tools') return aiToolRoutes.includes(pathname);
  if (url === '/hub') return hubRoutes.includes(pathname);
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
        ...
        <div className="max-w-[1400px] mx-auto px-4 md:px-6">
          <div className="h-14 flex items-center justify-between">
            <NavLink to="/" end className="flex items-center gap-2.5 flex-shrink-0 group" activeClassName="">
              ...
              <span className="font-display font-bold text-sm text-gradient tracking-tight hidden sm:inline">Lumina</span>
            </NavLink>
              ...
              {profile && (
                <motion.div whileHover={{ scale: 1.05 }}
                  ...
                  <div className="w-16 h-1.5 rounded-full bg-muted overflow-hidden">
                    <motion.div className="h-full rounded-full bg-primary" initial={{ width: 0 }} animate={{ width: `${levelProgress}%` }} transition={{ duration: 0.5 }} />
                  </div>
                </motion.div>
              )}

              <motion.button onClick={() => navigate('/study-session')} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                ...
                <span className="text-xs font-medium text-primary tabular-nums tracking-tight">{timerMins}:{String(timerSecs).padStart(2, '0')}</span>
              </motion.button>
              ...
              {profile && (
                <div className="hidden sm:flex items-center gap-3">
                  ...
                  <div className="w-8 h-8 rounded-full bg-muted overflow-hidden border border-border/20 transition-transform duration-300 hover:scale-110 cursor-pointer" onClick={() => navigate('/settings')}>
                    {profile.avatar_url ? <img src={profile.avatar_url} alt="" className="w-full h-full object-cover" /> : <div className="w-full h-full gradient-primary" />}
                  </div>
                </div>
              )}

              <button onClick={() => setMobileOpen(!mobileOpen)} className="md:hidden p-1.5 text-muted-foreground hover:text-foreground transition-all duration-200">
        ...
        <AnimatePresence>
          {mobileOpen && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
              transition={{ duration: 0.25, ease: [0.25, 0.1, 0.25, 1] }} className="md:hidden overflow-hidden border-t border-border/10">
              <div className="max-w-[1400px] mx-auto px-6 py-3 space-y-0.5">
                {visibleNav.map(item => {
                  ...
                  return (
                    <NavLink key={item.url} to={item.url} end={item.url === '/'}
                      ...
                      activeClassName="" onClick={() => setMobileOpen(false)}>
                ...
                {profile && (
                  <div className="flex items-center gap-4 px-3 py-2.5 sm:hidden">
                    ...
                    <div className="flex items-center gap-1 text-xs"><Flame className="w-3.5 h-3.5 text-warning" /><span className="text-warning font-semibold">{profile.streak_days}</span></div>
                    <div className="flex items-center gap-1 text-xs"><Coins className="w-3.5 h-3.5 text-xp" /><span className="text-xp font-semibold">{profile.coins}</span></div>
                  </div>
                )}
                <button onClick={signOut} className="w-full text-left px-3 py-2.5 text-sm text-destructive hover:bg-destructive/8 transition-all duration-150 rounded-xl flex items-center gap-2">
      ...
      <main className="flex-1 overflow-auto">
        <div className="max-w-[1200px] mx-auto px-4 md:px-6 py-6 md:py-8">{children}</div>
      </main>
    </div>
  );
};
