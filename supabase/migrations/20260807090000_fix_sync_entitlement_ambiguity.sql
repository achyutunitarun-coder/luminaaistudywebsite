-- Fix column-reference ambiguity in sync_dodo_entitlement_for_user.
-- The output table declares columns named `balance` and `plan`, which collide
-- with the table columns in `UPDATE ... SET balance = balance + ...`. Qualify
-- the table column references (same fix as 20260807070000 for the other
-- credit RPCs).
