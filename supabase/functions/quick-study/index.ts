...
serve(async (req) => {
  ...
  try {
    ...
    const prompt = `You are an expert tutor. Create a comprehensive quick study lesson on "${topic}" with 8-10 key concepts and 8 practice questions. Return ONLY valid JSON with no markdown fences: {"title": "...", "key_concepts": [{"concept": "name", "explanation": "detailed explanation 3-5 sentences"}], "practice_questions": [{"question": "...", "options": ["A","B","C","D"], "correct": 0, "explanation": "..."}]}
...
JSON:`;
    ...
    const response = await fetch(HF_API_URL, {
      ...
      body: JSON.stringify({
        inputs: prompt,
        parameters: { max_new_tokens: 2500, temperature: 0.5, return_full_text: false },
      }),
    });
