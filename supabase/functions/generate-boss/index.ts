  ...
  try {
    ...
    const response = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: "system", content: "You create educational boss battle questions for a gamified study app." },
          { role: "user", content: `Create a boss battle for "${topic}". Generate a boss name, emoji icon, and 5 challenging questions with 4 options each.` },
        ],
        tools: [{
          type: "function",
          function: {
