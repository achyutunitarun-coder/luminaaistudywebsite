...
function cleanJSON(raw: string): any {
  ...
  try { return JSON.parse(match[0]); } catch {
    try { return JSON.parse(match[0].replace(/,\s*([\]}])/g, "$1")); } catch { return null; }
  }
}
...
serve(async (req) => {
  ...
  try {
    ...
    return new Response(JSON.stringify({ error: "Failed to generate boss" }), { status: 502, headers: { ...corsHeaders, "Content-Type": "application/json" } });
  } catch (e) {
