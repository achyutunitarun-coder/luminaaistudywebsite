...
serve(async (req) => {
  ...
  try {
    ...
    const prompt = `You are a study planner. Generate a personalized daily study plan. Return ONLY valid JSON with no markdown fences: {"days": [{"day": 1, "date": "YYYY-MM-DD", "tasks": [{"subject": "...", "topic": "...", "duration_minutes": 30, "type": "study"}]}]}

Subjects: ${subjects.join(", ")}. Exam date: ${examDate}. Daily study time: ${dailyHours} hours.
